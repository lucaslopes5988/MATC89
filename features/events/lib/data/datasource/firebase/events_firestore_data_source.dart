...
class EventsFirestoreDataSource {
  ...
  Future<List<({EventDbDto dto, String id})>> getJoinedEvents(
    String userId,
  ) async {
    ...
    final records =
        snapshot.docs
            .map(
              (doc) => (dto: EventDbDto.fromFirestore(doc.data()), id: doc.id),
            )
            .where((record) => record.dto.startAt.compareTo(now) >= 0)
            .toList()
          ..sort((a, b) => a.dto.startAt.compareTo(b.dto.startAt));
